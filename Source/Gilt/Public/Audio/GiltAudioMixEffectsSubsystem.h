...
#include "GiltAudioMixEffectsSubsystem.generated.h"

class USoundControlBus;
class USoundControlBusMix;
USTRUCT()
struct FGiltAudioSubmixEffectsChain
{
	GENERATED_BODY()
 ...
 */
UCLASS()
class GILT_API UGiltAudioMixEffectsSubsystem : public UWorldSubsystem
{
	GENERATED_BODY()
...
protected:
	...
	UPROPERTY(Transient)
	TArray<FGiltAudioSubmixEffectsChain> HDRSubmixEffectChain;

	// Submix Effect hain Overrides to apply when HDR Audio is turned off
	UPROPERTY(Transient)
	TArray<FGiltAudioSubmixEffectsChain> LDRSubmixEffectChain;
