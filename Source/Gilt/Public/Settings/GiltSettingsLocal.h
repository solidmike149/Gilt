...
#include "Performance/GiltPerformanceStatTypes.h"
#include "GiltSettingsLocal.generated.h"


class USoundControlBusMix;
class UGiltLocalPlayer;
class USoundControlBus;
USTRUCT()
struct FGiltScalabilitySnapshot
{
	GENERATED_BODY()

	FGiltScalabilitySnapshot();
 ...
 */
UCLASS()
class GILT_API UGiltSettingsLocal : public UGameUserSettings
{
	GENERATED_BODY()
...
public:
	...
	EGiltStatDisplayMode GetPerfStatDisplayState(EGiltDisplayablePerformanceStat Stat) const;
	
	/** Sets the display mode for the specified performance stat */
	void SetPerfStatDisplayState(EGiltDisplayablePerformanceStat Stat, EGiltStatDisplayMode DisplayMode);

	/** Fired when the display state for a performance stat has changed, or the settings are applied */
	DECLARE_EVENT(UGiltSettingsLocal, FPerfStatSettingsChanged);
...
private:
	...
	UPROPERTY(Config)
	TMap<EGiltDisplayablePerformanceStat, EGiltStatDisplayMode> DisplayStatList;
...
private:
	...
	void OverrideQualityLevelsToScalabilityMode(const FGiltScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	FGiltScalabilitySnapshot DeviceDefaultScalabilitySettings;
...
private:
	...
	// Audio - Volume
public:
	DECLARE_EVENT_OneParam(UGiltSettingsLocal, FAudioDeviceChanged, const FString& /*DeviceId*/);
...
public:
	...
	DECLARE_EVENT_OneParam(UGiltSettingsLocal, FInputConfigDelegate, const FLoadedMappableConfigPair& /*Config*/);
	 ...
	 */
	void AddOrUpdateCustomKeyboardBindings(const FName MappingName, const FKey NewKey, UGiltLocalPlayer* LocalPlayer);
