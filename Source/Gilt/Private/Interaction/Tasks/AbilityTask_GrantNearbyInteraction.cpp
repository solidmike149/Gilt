...
void UAbilityTask_GrantNearbyInteraction::QueryInteractables()
{
	...
	if (World && ActorOwner)
	{
		...
		for(TScriptInterface<IGiltInteractableTargetInterface>& HighlightedActor : HighlightedActors)
		{
			HighlightedActor->HighlightInteractableActor();
		}
		...
		if (OverlapResults.Num() > 0)
		{
			...
			for (TScriptInterface<IGiltInteractableTargetInterface>& InteractiveTarget : InteractableTargets)
			{
				...
				InteractiveTarget->HighlightInteractableActor();

				HighlightedActors = InteractableTargets;
			}
