...
FGiltGameplayTags FGiltGameplayTags::GameplayTags;

void FGiltGameplayTags::InitializeNativeTags()
{
...
void FGiltGameplayTags::AddAllTags(UGameplayTagsManager& Manager)
{
	...
	AddTag(Status_Aim,"Status.Aim", "Target is aiming");
...
void FGiltGameplayTags::AddTag(FGameplayTag& OutTag, const ANSICHAR* TagName, const ANSICHAR* TagComment)
{
	OutTag = UGameplayTagsManager::Get().AddNativeGameplayTag(FName(TagName), FString(TEXT("(Native) ")) + FString(TagComment));
}

void FGiltGameplayTags::AddMovementModeTag(FGameplayTag& OutTag, const ANSICHAR* TagName, uint8 MovementMode)
{
...
FGameplayTag FGiltGameplayTags::FindTagByString(FString TagString, bool bMatchPartialString)
{
	...
	if (!Tag.IsValid() && bMatchPartialString)
	{
		...
		for (const FGameplayTag TestTag : AllTags)
		{
			...
			{
				UE_LOG(LogGilt, Display, TEXT("Could not find exact match for tag [%s] but found partial match on tag [%s]."), *TagString, *TestTag.ToString());
				Tag = TestTag;
				break;
			}
		}
