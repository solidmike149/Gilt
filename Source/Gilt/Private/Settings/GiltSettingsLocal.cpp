...
#include "Development/GiltPlatformEmulationSettings.h"
...
#if WITH_EDITOR
static TAutoConsoleVariable<bool> CVarApplyFrameRateSettingsInPIE(TEXT("Gilt.Settings.ApplyFrameRateSettingsInPIE"),
	false,
	TEXT("Should we apply frame rate settings in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyFrontEndPerformanceOptionsInPIE(TEXT("Gilt.Settings.ApplyFrontEndPerformanceOptionsInPIE"),
	false,
	TEXT("Do we apply front-end specific performance options in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyDeviceProfilesInPIE(TEXT("Gilt.Settings.ApplyDeviceProfilesInPIE"),
	false,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenTargetFps(
	TEXT("Gilt.DeviceProfile.Console.TargetFPS"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenFrameSyncType(
	TEXT("Gilt.DeviceProfile.Console.FrameSyncType"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileDefaultFrameRate(
	TEXT("Gilt.DeviceProfile.Mobile.DefaultFrameRate"),
	30,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileMaxFrameRate(
	TEXT("Gilt.DeviceProfile.Mobile.MaxFrameRate"),
	30,
...
static TAutoConsoleVariable<FString> CVarMobileQualityLimits(
	TEXT("Gilt.DeviceProfile.Mobile.OverallQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityLimits(
	TEXT("Gilt.DeviceProfile.Mobile.ResolutionQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityRecommendation(
	TEXT("Gilt.DeviceProfile.Mobile.ResolutionQualityRecommendation"),
	TEXT("0:75"),
...
FGiltScalabilitySnapshot::FGiltScalabilitySnapshot()
{
...
namespace GiltSettingsHelpers
{
	...
	void FillScalabilitySettingsFromDeviceProfile(FGiltScalabilitySnapshot& Mode, const FString& Suffix = FString())
	{
		static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
	...
	{
		const UGiltPlatformSpecificRenderingSettings* PlatformSettings = UGiltPlatformSpecificRenderingSettings::Get();
		...
		int32 LimitIndex = PossibleRates.FindLastByPredicate([=](const int32& TestRate)
		{
			...
			const bool bIsSupported = UGiltSettingsLocal::IsSupportedMobileFramePace(TestRate);

			return bAtOrBelowDesiredRate && bQualityDoesntExceedLimit && bIsSupported;
		});

		return PossibleRates.IsValidIndex(LimitIndex) ? PossibleRates[LimitIndex] : UGiltSettingsLocal::GetDefaultMobileFrameRate();
	}
...
UGiltSettingsLocal::UGiltSettingsLocal()
{
...
void UGiltSettingsLocal::SetToDefaults()
{
	Super::SetToDefaults();
...
void UGiltSettingsLocal::LoadSettings(bool bForceReload)
{
	Super::LoadSettings(bForceReload);

	// Console platforms use rhi.SyncInterval to limit framerate
	const UGiltPlatformSpecificRenderingSettings* PlatformSettings = UGiltPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EGiltFramePacingMode::ConsoleStyle)
	{
		FrameRateLimit = 0.0f;
	}
	...
	GiltSettingsHelpers::FillScalabilitySettingsFromDeviceProfile(DeviceDefaultScalabilitySettings);
...
void UGiltSettingsLocal::ResetToCurrentSettings()
{
...
void UGiltSettingsLocal::BeginDestroy()
{
...
UGiltSettingsLocal* UGiltSettingsLocal::Get()
{
	return GEngine ? CastChecked<UGiltSettingsLocal>(GEngine->GetGameUserSettings()) : nullptr;
}

void UGiltSettingsLocal::ConfirmVideoMode()
{
...
float UGiltSettingsLocal::GetEffectiveFrameRateLimit()
{
	const UGiltPlatformSpecificRenderingSettings* PlatformSettings = UGiltPlatformSpecificRenderingSettings::Get();
	...
	if (PlatformSettings->FramePacingMode == EGiltFramePacingMode::DesktopStyle)
	{
...
int32 UGiltSettingsLocal::GetHighestLevelOfAnyScalabilityChannel() const
{
	return GiltSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(ScalabilityQuality);
}

void UGiltSettingsLocal::OverrideQualityLevelsToScalabilityMode(const FGiltScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UGiltSettingsLocal::ClampQualityLevelsToDeviceProfile(const Scalability::FQualityLevels& ClampLevels, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UGiltSettingsLocal::OnExperienceLoaded()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UGiltSettingsLocal::OnHotfixDeviceProfileApplied()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UGiltSettingsLocal::ReapplyThingsDueToPossibleDeviceProfileChange()
{
	ApplyNonResolutionSettings();
}

void UGiltSettingsLocal::SetShouldUseFrontendPerformanceSettings(bool bInFrontEnd)
{
	bInFrontEndForPerformancePurposes = bInFrontEnd;
	UpdateEffectiveFrameRateLimit();
}

bool UGiltSettingsLocal::ShouldUseFrontendPerformanceSettings() const
{
#if WITH_EDITOR
...
EGiltStatDisplayMode UGiltSettingsLocal::GetPerfStatDisplayState(EGiltDisplayablePerformanceStat Stat) const
{
	if (const EGiltStatDisplayMode* pMode = DisplayStatList.Find(Stat))
	{
		return *pMode;
	}
	else
	{
		return EGiltStatDisplayMode::Hidden;
	}
}

void UGiltSettingsLocal::SetPerfStatDisplayState(EGiltDisplayablePerformanceStat Stat, EGiltStatDisplayMode DisplayMode)
{
	if (DisplayMode == EGiltStatDisplayMode::Hidden)
	{
...
float UGiltSettingsLocal::GetDisplayGamma() const
{
	return DisplayGamma;
}

void UGiltSettingsLocal::SetDisplayGamma(float InGamma)
{
	DisplayGamma = InGamma;
	ApplyDisplayGamma();
}

void UGiltSettingsLocal::ApplyDisplayGamma()
{
	if (GEngine)
	{
...
float UGiltSettingsLocal::GetFrameRateLimit_OnBattery() const
{
	return FrameRateLimit_OnBattery;
}

void UGiltSettingsLocal::SetFrameRateLimit_OnBattery(float NewLimitFPS)
{
	FrameRateLimit_OnBattery = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UGiltSettingsLocal::GetFrameRateLimit_InMenu() const
{
	return FrameRateLimit_InMenu;
}

void UGiltSettingsLocal::SetFrameRateLimit_InMenu(float NewLimitFPS)
{
	FrameRateLimit_InMenu = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UGiltSettingsLocal::GetFrameRateLimit_WhenBackgrounded() const
{
	return FrameRateLimit_WhenBackgrounded;
}

void UGiltSettingsLocal::SetFrameRateLimit_WhenBackgrounded(float NewLimitFPS)
{
	FrameRateLimit_WhenBackgrounded = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UGiltSettingsLocal::GetFrameRateLimit_Always() const
{
	return GetFrameRateLimit();
}

void UGiltSettingsLocal::SetFrameRateLimit_Always(float NewLimitFPS)
{
	SetFrameRateLimit(NewLimitFPS);
	UpdateEffectiveFrameRateLimit();
}

void UGiltSettingsLocal::UpdateEffectiveFrameRateLimit()
{
...
int32 UGiltSettingsLocal::GetDefaultMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileDefaultFrameRate.GetValueOnGameThread();
}

int32 UGiltSettingsLocal::GetMaxMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileMaxFrameRate.GetValueOnGameThread();
}

bool UGiltSettingsLocal::IsSupportedMobileFramePace(int32 TestFPS)
{
...
int32 UGiltSettingsLocal::GetFirstFrameRateWithQualityLimit() const
{
	return GiltSettingsHelpers::GetFirstFrameRateWithQualityLimit();
}

int32 UGiltSettingsLocal::GetLowestQualityWithFrameRateLimit() const
{
	return GiltSettingsHelpers::GetLowestQualityWithFrameRateLimit();
}

void UGiltSettingsLocal::ResetToMobileDeviceDefaults()
{
	// Reset frame rate
...
int32 UGiltSettingsLocal::GetMaxSupportedOverallQualityLevel() const
{
	const UGiltPlatformSpecificRenderingSettings* PlatformSettings = UGiltPlatformSpecificRenderingSettings::Get();
	if ((PlatformSettings->FramePacingMode == EGiltFramePacingMode::MobileStyle) && DeviceDefaultScalabilitySettings.bHasOverrides)
	{
		return GiltSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(DeviceDefaultScalabilitySettings.Qualities);
	}
	else
	{
		return 3;
...
void UGiltSettingsLocal::SetMobileFPSMode(int32 NewLimitFPS)
{
	const UGiltPlatformSpecificRenderingSettings* PlatformSettings = UGiltPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EGiltFramePacingMode::MobileStyle)
	{
...
void UGiltSettingsLocal::SetDesiredMobileFrameRateLimit(int32 NewLimitFPS)
{
...
void UGiltSettingsLocal::ClampMobileFPSQualityLevels(bool bWriteBack)
{
	const UGiltPlatformSpecificRenderingSettings* PlatformSettings = UGiltPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EGiltFramePacingMode::MobileStyle)
	{
		const int32 QualityLimit = GiltSettingsHelpers::GetApplicableOverallQualityLimit(DesiredMobileFrameRateLimit);
...
void UGiltSettingsLocal::ClampMobileQuality()
{
	const UGiltPlatformSpecificRenderingSettings* PlatformSettings = UGiltPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EGiltFramePacingMode::MobileStyle)
	{
...
void UGiltSettingsLocal::ClampMobileResolutionQuality(int32 TargetFPS)
{
	// Clamp mobile resolution quality
	float MaxMobileResQuality = GiltSettingsHelpers::GetApplicableResolutionQualityLimit(TargetFPS);
...
void UGiltSettingsLocal::RemapMobileResolutionQuality(int32 FromFPS, int32 ToFPS)
{
	...
	float MaxScaleValue = 0.0f;
	GetResolutionScaleInformationEx(CurrentScaleNormalized, CurrentScaleValue, MinScaleValue, MaxScaleValue);
	float FromMaxMobileResQuality = GiltSettingsHelpers::GetApplicableResolutionQualityLimit(FromFPS);
	float ToMaxMobileResQuality = GiltSettingsHelpers::GetApplicableResolutionQualityLimit(ToFPS);
...
FString UGiltSettingsLocal::GetDesiredDeviceProfileQualitySuffix() const
{
	return DesiredUserChosenDeviceProfileSuffix;
}

void UGiltSettingsLocal::SetDesiredDeviceProfileQualitySuffix(const FString& InDesiredSuffix)
{
	DesiredUserChosenDeviceProfileSuffix = InDesiredSuffix;
}

void UGiltSettingsLocal::SetHeadphoneModeEnabled(bool bEnabled)
{
...
bool UGiltSettingsLocal::IsHeadphoneModeEnabled() const
{
	return bUseHeadphoneMode;
}

bool UGiltSettingsLocal::CanModifyHeadphoneModeEnabled() const
{
	static IConsoleVariable* BinauralSpatializationDisabledCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("au.DisableBinauralSpatialization"));
	...
	const bool bBinauralSettingControlledByOS = GiltSettingsHelpers::HasPlatformTrait(TAG_Platform_Trait_BinauralSettingControlledByOS);

	return bHRTFOptionAvailable && !bBinauralSettingControlledByOS;
}

bool UGiltSettingsLocal::IsHDRAudioModeEnabled() const
{
	return bUseHDRAudioMode;
}

void UGiltSettingsLocal::SetHDRAudioModeEnabled(bool bEnabled)
{
	...
	if (GEngine)
	{
		...
		{
			if (UGiltAudioMixEffectsSubsystem* GiltAudioMixEffectsSubsystem = World->GetSubsystem<UGiltAudioMixEffectsSubsystem>())
			{
				GiltAudioMixEffectsSubsystem->ApplyDynamicRangeEffectsChains(bEnabled);
			}
		}
	}
}

bool UGiltSettingsLocal::CanRunAutoBenchmark() const
{
	const UGiltPlatformSpecificRenderingSettings* PlatformSettings = UGiltPlatformSpecificRenderingSettings::Get();
	return PlatformSettings->bSupportsAutomaticVideoQualityBenchmark;
}

bool UGiltSettingsLocal::ShouldRunAutoBenchmarkAtStartup() const
{
...
void UGiltSettingsLocal::RunAutoBenchmark(bool bSaveImmediately)
{
...
void UGiltSettingsLocal::ApplyScalabilitySettings()
{
	Scalability::SetQualityLevels(ScalabilityQuality);
}

float UGiltSettingsLocal::GetOverallVolume() const
{
	return OverallVolume;
}

void UGiltSettingsLocal::SetOverallVolume(float InVolume)
{
...
float UGiltSettingsLocal::GetMusicVolume() const
{
	return MusicVolume;
}

void UGiltSettingsLocal::SetMusicVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UGiltSettingsLocal::GetSoundFXVolume() const
{
	return SoundFXVolume;
}

void UGiltSettingsLocal::SetSoundFXVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UGiltSettingsLocal::GetDialogueVolume() const
{
	return DialogueVolume;
}

void UGiltSettingsLocal::SetDialogueVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UGiltSettingsLocal::GetVoiceChatVolume() const
{
	return VoiceChatVolume;
}

void UGiltSettingsLocal::SetVoiceChatVolume(float InVolume)
{
	// Cache the incoming volume value
...
void UGiltSettingsLocal::SetVolumeForControlBus(USoundControlBus* InSoundControlBus, float InVolume)
{
	// Check to see if references to the control buses and control bus mixes have been loaded yet
...
void UGiltSettingsLocal::SetAudioOutputDeviceId(const FString& InAudioOutputDeviceId)
{
	AudioOutputDeviceId = InAudioOutputDeviceId;
	OnAudioOutputDeviceChanged.Broadcast(InAudioOutputDeviceId);
}

void UGiltSettingsLocal::ApplySafeZoneScale()
{
	SSafeZone::SetGlobalSafeZoneScale(GetSafeZone());
}

void UGiltSettingsLocal::ApplyNonResolutionSettings()
{
...
int32 UGiltSettingsLocal::GetOverallScalabilityLevel() const
{
...
void UGiltSettingsLocal::SetOverallScalabilityLevel(int32 Value)
{
	...
	if (PlatformSettings->FramePacingMode == EGiltFramePacingMode::MobileStyle)
	{
		...
		const int32 ConstrainedFrameRateLimit = GiltSettingsHelpers::ConstrainFrameRateToBeCompatibleWithOverallQuality(DesiredMobileFrameRateLimit, Value);
...
void UGiltSettingsLocal::SetControllerPlatform(const FName InControllerPlatform)
{
...
FName UGiltSettingsLocal::GetControllerPlatform() const
{
	return ControllerPlatform;
}

void UGiltSettingsLocal::RegisterInputConfig(ECommonInputType Type, const UPlayerMappableInputConfig* NewConfig, const bool bIsActive)
{
	if (NewConfig)
	{
...
int32 UGiltSettingsLocal::UnregisterInputConfig(const UPlayerMappableInputConfig* ConfigToRemove)
{
	if (ConfigToRemove)
	{
...
void UGiltSettingsLocal::ActivateInputConfig(const UPlayerMappableInputConfig* Config)
{
	if (Config)
	{
...
void UGiltSettingsLocal::DeactivateInputConfig(const UPlayerMappableInputConfig* Config)
{
	if (Config)
	{
		const int32 ExistingConfigIdx = RegisteredInputConfigs.IndexOfByPredicate( [&Config](const FLoadedMappableConfigPair& Pair) { return Pair.Config == Config; } );
...
const UPlayerMappableInputConfig* UGiltSettingsLocal::GetInputConfigByName(FName ConfigName) const
{
...
void UGiltSettingsLocal::GetRegisteredInputConfigsOfType(ECommonInputType Type, TArray<FLoadedMappableConfigPair>& OutArray) const
{
	OutArray.Empty();
...
void UGiltSettingsLocal::AddOrUpdateCustomKeyboardBindings(const FName MappingName, const FKey NewKey, UGiltLocalPlayer* LocalPlayer)
{
...
void UGiltSettingsLocal::LoadUserControlBusMix()
{
	if (GEngine)
	{
		if (const UWorld* World = GEngine->GetCurrentPlayWorld())
		{
			if (const UGiltAudioSettings* GiltAudioSettings = GetDefault<UGiltAudioSettings>())
			{
				...
				if (UObject* ObjPath = GiltAudioSettings->UserSettingsControlBusMix.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBusMix, TEXT("User Settings Control Bus Mix reference missing from Gilt Audio Settings."));
					}
				}
			}
		}
...
void UGiltSettingsLocal::OnAppActivationStateChanged(bool bIsActive)
{
	// We might want to adjust the frame rate when the app loses/gains focus on multi-window platforms
	UpdateEffectiveFrameRateLimit();
}

void UGiltSettingsLocal::UpdateGameModeDeviceProfileAndFps()
{
#if WITH_EDITOR
	...
	const UGiltPlatformSpecificRenderingSettings* PlatformSettings = UGiltPlatformSpecificRenderingSettings::Get();
	const TArray<FGiltQualityDeviceProfileVariant>& UserFacingVariants = PlatformSettings->UserFacingDeviceProfileOptions;
	...
	int32 SuffixIndex = UserFacingVariants.IndexOfByPredicate([&](const FGiltQualityDeviceProfileVariant& Data){ return Data.DeviceProfileSuffix == UserChosenDeviceProfileSuffix; });
	...
	if (GIsEditor)
	{
		const UGiltPlatformEmulationSettings* Settings = GetDefault<UGiltPlatformEmulationSettings>();
	...
	{
	case EGiltFramePacingMode::MobileStyle:
		UpdateMobileFramePacing();
		break;
	case EGiltFramePacingMode::ConsoleStyle:
		UpdateConsoleFramePacing();
		break;
	case EGiltFramePacingMode::DesktopStyle:
		UpdateDesktopFramePacing();
		break;
	}
}

void UGiltSettingsLocal::UpdateConsoleFramePacing()
{
...
void UGiltSettingsLocal::UpdateDesktopFramePacing()
{
...
void UGiltSettingsLocal::UpdateMobileFramePacing()
{
...
void UGiltSettingsLocal::UpdateDynamicResFrameTime(float TargetFPS)
{
