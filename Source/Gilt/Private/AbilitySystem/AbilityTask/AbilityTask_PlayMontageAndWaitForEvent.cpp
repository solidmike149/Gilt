...
#include "AbilitySystem/AbilityTask/AbilityTask_PlayMontageAndWaitForEvent.h"
...
UGiltAbilitySystemComponent* UAbilityTask_PlayMontageAndWaitForEvent::GetTargetASC()
{
	return Cast<UGiltAbilitySystemComponent>(AbilitySystemComponent);
}
...
void UAbilityTask_PlayMontageAndWaitForEvent::Activate()
{
	...
	UGiltAbilitySystemComponent* GiltASC = GetTargetASC();
	...
	if (GiltASC)
	{
		...
		if (AnimInstance != nullptr)
		{
			...
			EventHandle = GiltASC->AddGameplayEventTagContainerDelegate(EventTags, FGameplayEventTagMulticastDelegate::FDelegate::CreateUObject(this, &UAbilityTask_PlayMontageAndWaitForEvent::OnGameplayEvent));

			if (GiltASC->PlayMontage(Ability, Ability->GetCurrentActivationInfo(), MontageToPlay, Rate, StartSection) > 0.f)
			{
...
void UAbilityTask_PlayMontageAndWaitForEvent::OnDestroy(bool AbilityEnded)
{
	...
	if (GiltASC)
	{
		GiltASC->RemoveGameplayEventTagContainerDelegate(EventTags, EventHandle);
	}
