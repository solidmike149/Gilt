...
enum class EGiltEditorLoadMode
{
...
namespace GiltGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("Gilt.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via GiltGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UGiltGameplayCueManager::DumpGameplayCues));

	static EGiltEditorLoadMode LoadMode = EGiltEditorLoadMode::LoadUpfront;
}
...
UGiltGameplayCueManager::UGiltGameplayCueManager(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UGiltGameplayCueManager* UGiltGameplayCueManager::Get()
{
	return Cast<UGiltGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UGiltGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UGiltGameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(LogGilt, Warning, TEXT("UGiltGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UGiltGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (GiltGameplayCueManagerCvars::LoadMode)
	{
	case EGiltEditorLoadMode::LoadUpfront:
		return true;
	case EGiltEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case EGiltEditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UGiltGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UGiltGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UGiltGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UGiltGameplayCueManager* GCM = Cast<UGiltGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogGilt, Error, TEXT("DumpGameplayCues failed. No UGiltGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogGilt, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogGilt, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogGilt, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogGilt, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogGilt, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogGilt, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(LogGilt, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(LogGilt, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(LogGilt, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(LogGilt, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(LogGilt, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(LogGilt, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UGiltGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UGiltGameplayCueManager* StrongThis = Get())
					{
...
void UGiltGameplayCueManager::HandlePostGarbageCollect()
{
...
void UGiltGameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(LogGilt, Warning, TEXT("UGiltGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UGiltGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (GiltGameplayCueManagerCvars::LoadMode)
	{
	case EGiltEditorLoadMode::LoadUpfront:
...
void UGiltGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UGiltGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UGiltGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UGiltGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UGiltGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
	const bool bClientDelayLoadGameplayCues = true;
	return !IsRunningDedicatedServer() && bClientDelayLoadGameplayCues;
}

const FPrimaryAssetType UGiltAssetManager_GameplayCueRefsType = TEXT("GameplayCueRefs");
const FName UGiltAssetManager_GameplayCueRefsName = TEXT("GameplayCueReferences");
const FName UGiltAssetManager_LoadStateClient = FName(TEXT("Client"));

void UGiltGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
	...
	BundleData.AddBundleAssetsTruncated(UGiltAssetManager_LoadStateClient, CuePaths);

	FPrimaryAssetId PrimaryAssetId = FPrimaryAssetId(UGiltAssetManager_GameplayCueRefsType, UGiltAssetManager_GameplayCueRefsName);
